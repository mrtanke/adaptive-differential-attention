...
class DifferentialAttentionV1(nn.Module):
 ...
 def forward(s,x,return_attention=False):
  b,t,d=x.shape;q=s.rope(s.q(x).view(b,t,2*s.h,s.k));k=s.rope(s.kp(x).view(b,t,2*s.h,s.k));v=s.v(x).view(b,t,s.h,2*s.k);maps=F.softmax(((q.transpose(1,2)@k.transpose(1,2).transpose(-2,-1))*s.k**-.5).masked_fill(~causal_mask(t,x.device),float("-inf")),dim=-1,dtype=torch.float32).to(x.dtype).view(b,s.h,2,t,t);w=maps[:,:,0]-s.lambda_full().to(x.dtype)*maps[:,:,1];y=s.o((s.subln(w@v.transpose(1,2))*(1-s.lambda_init)).transpose(1,2).reshape(b,t,d));return(y,w)if return_attention else y
